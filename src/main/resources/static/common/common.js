...
function header() {
    ...
    header += '</div>\n' +
        ...
        '                        <a class="user-name" href="myQuestionnaires.html"><i class="icon questions-icon"><em></em></i><span>我的问卷</span></a>\n' +
        '                    </dt>\n' +
        '                    <dd class="line"></dd>\n' +
        '                </dl>\n' +
...
function dateChange(time) {
    var date1 = new Date(timeFormat(time));
 ...
 * @param time
 */
function checkDateout(timeBegin,timeEnd) {
    var d_begin = new Date(timeBegin);
    var d_end = new Date(timeEnd);
    var year1 = d_begin.getFullYear();
    var month1 = d_begin.getMonth() + 1;     //月
    var d_beginay1 = d_begin.getDate();            //日
    var hh1 = d_begin.getHours();            //时
    var mm1 = d_begin.getMinutes();          //分
    var ss1 = d_begin.getSecond_begins();           //秒

    var year2 = d_end.getFullYear();//年
    var month2 = d_end.getMonth() + 1;     //月
    var d_enday2 = d_end.getDate();            //日
    var hh2 = d_end.getHours();            //时
    var mm2 = d_end.getMinutes();          //分
    var ss2 = d_end.getSecond_ends();           //秒
    if(year1>year2)
        return false
    if(month1>month2)
        return false
    if(d_enday1>d_enday2)
        return false
    if(hh1>hh2)
        return false
    if(mm1>mm2)
