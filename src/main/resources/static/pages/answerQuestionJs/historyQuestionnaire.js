...
function getHistoryQuest() {
    var url = '/queryDeletedQuestionnaireList';
    var data = {
    ...
    if (result.code == "666") {
        ...
        if (questionInfo.length) {
            ...
            for (var i = 0; i < questionInfo.length; i++) {
                ...
                text += "        <span>" + timeFormat(new Date(questionInfo[i].endTime)) + "</span>";
                text += "    </td>";
                ...
                text += "    </td>";
                text += "    <td style='width: 80px;cursor: pointer' id='getRecoverRunModal" + i + "' align=\"center\" onclick=\"recoverRun(" + "'" + questionInfo[i].id + "','" + i +"','"+questionInfo[i].projectId+"')\">";
...
function deleteHistoryQuest(id) {
    ...
    layer.confirm('您确认要删除此问卷吗？', {
        ...
        var url = '/deleteQuestionnaireById/'+id;
...
function recoverRunBtn() {
    ...
    if (endTimeModal == "") {
        ...
        var data = {
            ...
            "projectId": window.recoverRunprojectId,
            "createdBy": getCookie('userName'),
            'status': 2
        }
