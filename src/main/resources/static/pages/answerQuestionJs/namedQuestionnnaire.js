...
function getProjectBaseInfo() {
    commonAjaxPost(true,'/queryQuestionnaireById/'+questionId,undefined,function (res) {
         $('#questionName').val(res.data.questionName);
         $('#questionContent').val(res.data.questionContent);
        var startTime = timeFormat(new Date(res.data.startTime));
        var dateAfterNow = timeFormat(new Date(res.data.endTime));
...
function quickCreate() {
    ...
    if (getCookie('isEdit') != '2') {
        ...
        if(questionId){
            ...
            localStorage.setItem('questionInfo', JSON.stringify(da))
            window.location.href = 'designQuestionnaire.html?questionId='+questionId
        } else {
            var da = {
                'questionName': questionName,
                'questionContent': questionContent,
