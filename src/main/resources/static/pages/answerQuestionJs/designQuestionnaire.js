...
$(function () {
    ...
    if (questionInfo) {
        $('.questionTitle').text(questionInfo.questionName); //获取问卷名称
        $('#pater_desc').html(questionInfo.questionContent);//获取问卷说明
        flag = true
    }
    if (getCookie('isEdit') === '2') {
        questionId = getCookie('historyQuestionId')
        commonAjaxPost(true, '/queryQuestionnaireById/' + getCookie('historyQuestionId'), undefined, queryQuestionnaireAllSuccess)
    } else {
        var urlObj = GetRequest();
        if (Object.keys(urlObj).length > 0) {
            if (urlObj.questionId) {
                setCookie('editQuestion', true)
                commonAjaxPost(true, '/queryQuestionnaireById/' + urlObj.questionId, undefined, queryQuestionnaireAllSuccess)
            }
        }
    }
});
...
function editFinish() {
    ...
    if (questionList.length == 0) {
        ...
        if (localStorage.getItem('projectIdForCreate')) {
            questionInfo.projectId = localStorage.getItem('projectIdForCreate')
        }
        questionInfo.questionName = $('.questionTitle').text(); //获取问卷名称
        questionInfo.questionContent = $('#pater_desc').html();//获取问卷说明
        questionList.forEach(item => {
            item.required = item.important === '必答题' ? 1 : 0
        })
        questionInfo.questionList = questionList
        var url = ''
        var cookie = getCookie('editQuestion');
        if (cookie) {
            url = '/editQuestionnaire';
            questionInfo.id = questionIdForChange
        } else {
            url = '/addQuestionnaire';
        }
        window.questionName = questionInfo.questionName
        commonAjaxPost(true, url, questionInfo, addQuestionnaireSuccess)
    }
}
...
function addQuestionnaireSuccess(res) {
    if (res.code == '666') {
        ...
        deleteCookie('previewId');
        deleteCookie('editQuestion')
        deleteCookie('historyQuestionId')
        ...
        judgeQuestionId();
        questionList = [];
        window.location.href = 'sendQuestionnaire.html?questionId=' + res.data
...
function queryQuestionnaireAllSuccess(res) {
    if (res.code == '666') {
        ...
        if (!flag) {
            $('.questionTitle').text(res.data.questionName); //获取问卷名称
            $('#pater_desc').html(res.data.questionContent);//获取问卷说明
        }
...
function showQuestion(question) {
    ...
    var index = String(question.questionType);
    if (index == "-1") {
        return;
    }
    question.important = question.required === 1 ? '必答题' : '取消必答题'
    var movie_box = '<div class="movie_box" style="border: 1px solid rgb(255, 255, 255);"></div>';
    var Grade = $(".yd_box").find(".movie_box").length + 1;
    switch (index) {
        case "0": //单选
